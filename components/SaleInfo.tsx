...
export default function SaleInfo({ nft }: Props) {
    ...
    async function checkAndProvideApproval() {
        // Check if the marketplace address is approved
        const hasApproval = await nftCollection?.call(
            "isApprovedForAll",
            [nft.owner, MARKETPLACE_ADDRESS]  // Pass arguments as an array
        );
    
        // If not approved, set the approval
        if (!hasApproval) {
            const txResult = await nftCollection?.call(
                "setApprovalForAll",
                [MARKETPLACE_ADDRESS, true]  // Pass arguments as an array
            );
